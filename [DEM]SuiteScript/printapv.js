	/**
	*	Accounts Payable Voucher Print out
	*
	...
	**/
	function apv(request, response){
		...
		subExpenseCount = rec.getLineItemCount(subExpenseID);	//Counts the Line Expense
		...
		subID = subExpenseID;									//selects the expense tab line
		...
		for(var i = 1; i <= subCount; i++){							
			...
			if(subID == 'expense' && subExpenseCount > 0)		//if selected tab is "expense" and expense count is greater than zero
			{
				...
				department = rec.getLineItemValue(subID, 'department', i);
				dept = rec.getLineItemValue(subID, 'department', i);
				var dept1 = nlapiLoadRecord('department',dept);
				dept2 = dept1.getFieldValue('custrecord832');//alias
				dept3 = (dept2 == null) ? " " : dept2;//concatanate with je name + alias
				...
				itemCat = accountRec.getFieldValue('acctnumber');
				parnt = accountRec.getFieldText('parent');
				account = parnt.substring(10) + " : " + accountRec.getFieldValue('acctname') + " " + dept3;
			...
			/***#### FOR LINE ITEM TAB ####****/
				...
				department = (rec.getLineItemValue(subID, 'department', i) == null) ? department: rec.getLineItemValue(subID, 'department', i);
		...
		if(subCount > 0){
			for(var i in hashtable)
			{
				...
				if(parseFloat(hashtable[i]) > 0)
				{
					
					journalEntry += '<tr>';
					journalEntry += "<td>" + department1 + "-" +account_id + '</td>';	//account code wew
					//journalEntry += "<td><p class='wide2 textCenter'>" + department1 + '</p></td>'; //department code
				...
				{
					journalEntry += '<tr>';
					journalEntry += "<td>"+ department1 + "-" +account_id + '</td>';//wew
					//journalEntry += "<td><p class='wide2 textCenter'>"+department1+"</p></td>";//wew
					journalEntry += "<td>"+principal1+"</td>";
					journalEntry += "<td><p class='indent'>" + account_name + '</p></td>';
					//journalEntry += "<td class='entry'>" + account_name+ '</td>';	//account name
					journalEntry += "<td></td>";
					journalEntry += "<td><p align = 'right'>" + addCommas(nlapiFormatCurrency(hashtable[i] * -1)) + '</p></td>';//
				}
				journalEntry += '</tr>';
			}
		...
		var capitals = '';
		var adddress = vendor;
		//var dranix_addr = rec.getFieldText('location').split(':');
