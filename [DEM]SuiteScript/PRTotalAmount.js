...
function PrTotalAmount(type,name){ //validate line - add
	...
	if(!isDuplicateItem(currentitem,currentunit)){
		if(sublist_fields.Approve1 == 'T'){

			if(sublist_fields.Vendor1 != '' && sublist_fields.Price1 != '' && sublist_fields.Days1 != ''){
				finalVendor = sublist_fields.Vendor1;
				finalTerms = sublist_fields.Days1;

				if(finalVendor != '' && finalVendor != sublist_fields.FinalVendor){
					finalTaxCode = nlapiLookupField('vendor', finalVendor, 'custentity54') || '6';
					nlapiSetCurrentLineItemValue(itemSublist,itemSublist_fieldTaxCode, finalTaxCode);
				}

				compute = computation(sublist_fields.QtyOrder,sublist_fields.Price1,getCurrentLineValue().TaxRate);
			}else{
				alert('Please input Vendor1 or Price1 or Days1');
				return false;
			}
			

		}else if(sublist_fields.Approve2 == 'T'){

			if(sublist_fields.Vendor2 != '' && sublist_fields.Price2 != '' && sublist_fields.Days2 != ''){
				...
				compute = computation(sublist_fields.QtyOrder,sublist_fields.Price2,getCurrentLineValue().TaxRate);
			}else{
				alert('Please input Vendor2 or Price2 or Days2');
				return false;
			}
			

		}else if(sublist_fields.Approve3 == 'T'){

			if(sublist_fields.Vendor3 != '' && sublist_fields.Price3 != '' && sublist_fields.Days3 != ''){
				...
				compute = computation(sublist_fields.QtyOrder,sublist_fields.Price3,getCurrentLineValue().TaxRate);
			}else{
				alert('Please input Vendor3 or Price3 or Days3');
				return false;
			}
		...
		nlapiSetCurrentLineItemValue(itemSublist, itemSublist_fieldPaymentType, getPaymentType(finalTerms));
...
function saveRecord() {
	...
	if(nlapiGetLineItemCount(itemSublist) == 0)
	{
		alert('You must enter at least one line item for this transaction.');
		return false;
	}
	if(nlapiGetFieldValue('custrecord_deliveredto')=='' || nlapiGetFieldValue('custrecord_deliveredto')==null) {
		alert('Input value for Delivered To');
		return false;
	}

	if(nlapiGetFieldValue('custrecord41')=='21' || nlapiGetFieldValue('custrecord41')=='14') {
		if(nlapiGetFieldValue('custrecord879')=='' || nlapiGetFieldValue('custrecord879')==null) {
			alert('Input value for Plate No.');
			return false;
		}
	}
	
	if(nlapiGetFieldValue('custrecord886')=='' || nlapiGetFieldValue('custrecord886')==null) {
		alert('Input value for Purpose');
		return false;
	}
	return true;
}
...
function computation(quantityOrder,canvassPrice,taxrate){
	...
	var ucost = parseFloat(canvassPrice) / taxRate;
	ComputeObject.UnitCost = ucost.toFixed(5);
...
function getPaymentType(terms){
	var paymentType = '2';

	if(terms == '5' || terms == '6' || terms == '46' || terms == '21'){ // 5 = COD, 6 = CWO, 46 = PDC 30 Days, 21 = PDC 45 Days
		paymentType = '1'; // 1 = Advance Payment
	}else{
		if(nlapiGetCurrentLineItemValue(itemSublist, itemSublist_fieldPaymentType) == ''){
