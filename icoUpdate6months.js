function setICO(rec_type, rec_id)
{
	...
	for(var i = 0; i < icoList.length; i++){	
		...
		if(loc != ''){
			...
			var icoQuantity = nlapiSearchRecord('transaction', 'customsearch238', filter, columns);	
			if(icoQuantity != null)
			{
				var quantity = (icoQuantity[0].getValue('quantity', null, 'sum')/6); // monthly average sales 
