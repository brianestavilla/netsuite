function AutoCreatePR(type,form){
	
	if(type == 'approve'){ 
		//var statuss = nlapiGetFieldValue('custrecord670'); //get approval status
		
		//if(statuss == '2'){ //2 = Approve
			...
			for(var j = 1; j <= laborcostlinecount; j++){
				...
				if(laborrepairtype == '2'){ //1 = In-house, 2 = Job out
					...
					createPR.setLineItemValue('recmachcustrecord33','custrecord48',linecounter,noofdays); //set quantity to pr
					createPR.setLineItemValue('recmachcustrecord33','custrecord58',linecounter,rate); //set unitcost to pr
