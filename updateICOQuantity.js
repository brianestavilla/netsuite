function setICO(rec_type, rec_id)
{
	...
	for(var i = 0; i < icoList.length; i++){	
		...
		if(loc != ''){
			...
			var icoQuantity = nlapiSearchRecord('transaction', 'customsearch240', filter, columns);	
			if(icoQuantity != null)
			{
				var quantity = (icoQuantity[0].getValue('quantity', null, 'sum')/3); // monthly average sales 
